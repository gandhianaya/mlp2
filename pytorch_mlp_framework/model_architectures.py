...
# Batch Normalization Block (BN Only)
class ConvolutionalBNBlock(nn.Module):
    def __init__(self, input_shape, num_filters, kernel_size, padding, bias, dilation):
        super(ConvolutionalBNBlock, self).__init__()
    ...
    def build_module(self):
        ...
        # First convolution + Batch Normalization + Leaky ReLU
        self.layer_dict['conv_0'] = nn.Conv2d(in_channels=out.shape[1], out_channels=self.num_filters, bias=self.bias,
                                              kernel_size=self.kernel_size, dilation=self.dilation,
                                              padding=self.padding, stride=1)
        self.layer_dict['bn_0'] = nn.BatchNorm2d(num_features=self.num_filters)  # BatchNorm

        out = self.layer_dict['conv_0'].forward(out)
        out = self.layer_dict['bn_0'].forward(out)  # Batch Normalization
        out = F.leaky_relu(out)

        # Second convolution + Batch Normalization + Leaky ReLU
        self.layer_dict['conv_1'] = nn.Conv2d(in_channels=out.shape[1], out_channels=self.num_filters, bias=self.bias,
                                              kernel_size=self.kernel_size, dilation=self.dilation,
                                              padding=self.padding, stride=1)
        self.layer_dict['bn_1'] = nn.BatchNorm2d(num_features=self.num_filters)  # Add BatchNorm

        out = self.layer_dict['conv_1'].forward(out)
        out = self.layer_dict['bn_1'].forward(out)  # Batch Normalization
        out = F.leaky_relu(out)
...
class ConvolutionalResidualBlock(nn.Module):
    ...
    def build_module(self):
        ...
        self.layer_dict['conv_0'] = nn.Conv2d(in_channels=out.shape[1], out_channels=self.num_filters, bias=self.bias,
                                              kernel_size=self.kernel_size, dilation=self.dilation,
                                              padding=self.padding, stride=1)
        self.layer_dict['bn_0'] = nn.BatchNorm2d(num_features=self.num_filters)  # BatchNorm for conv_0
        
        out = self.layer_dict['conv_0'].forward(out)
        out = self.layer_dict['bn_0'].forward(out)  # Apply Batch Normalization
        out = F.leaky_relu(out)  # Apply Leaky ReLU

        # Second Convolution + LeakyReLU
        self.layer_dict['conv_1'] = nn.Conv2d(in_channels=out.shape[1], out_channels=self.num_filters, bias=self.bias,
                                              kernel_size=self.kernel_size, dilation=self.dilation,
                                              padding=self.padding, stride=1)
        self.layer_dict['bn_1'] = nn.BatchNorm2d(num_features=self.num_filters)  # BatchNorm after conv_1
        
        out = self.layer_dict['conv_1'].forward(out)
        out = self.layer_dict['bn_1'].forward(out)  # Apply Batch Normalization
        out = F.leaky_relu(out)  # Apply Leaky ReLU
    ...
    def forward(self, x):
        ...
        out += shortcut # Add residual connection
        out = F.leaky_relu(out)
...
# Batch Normalization with Dimensionality Reduction (Average Pooling)
class ConvolutionalBNDimReductionBlock(nn.Module):
    def __init__(self, input_shape, num_filters, kernel_size, padding, bias, dilation, reduction_factor):
        super(ConvolutionalBNDimReductionBlock, self).__init__()
    ...
    def build_module(self):
        ...
        self.layer_dict['conv_0'] = nn.Conv2d(in_channels=out.shape[1], out_channels=self.num_filters, bias=self.bias,
                                              kernel_size=self.kernel_size, dilation=self.dilation,
                                              padding=self.padding, stride=1)
        self.layer_dict['bn_0'] = nn.BatchNorm2d(num_features=self.num_filters) # Add BatchNorm after conv_0
        
        out = self.layer_dict['conv_0'].forward(out)
        out = self.layer_dict['bn_0'].forward(out) # Apply BatchNorm
        out = F.leaky_relu(out)
        
        # Average Pooling for downsampling
        out = F.avg_pool2d(out, self.reduction_factor)

        # Second Convolution + Batch Normalization + LeakyReLU
        self.layer_dict['conv_1'] = nn.Conv2d(in_channels=out.shape[1], out_channels=self.num_filters, bias=self.bias,
                                              kernel_size=self.kernel_size, dilation=self.dilation,
                                              padding=self.padding, stride=1)
        self.layer_dict['bn_1'] = nn.BatchNorm2d(num_features=self.num_filters)  # Add BatchNorm after conv_1

        out = self.layer_dict['conv_1'].forward(out)
        out = self.layer_dict['bn_1'].forward(out) # Apply BatchNorm
        out = F.leaky_relu(out)
    ...
    def forward(self, x):
        ...
        # Dimensionality reduction using Average Pooling
        out = F.avg_pool2d(out, self.reduction_factor)
